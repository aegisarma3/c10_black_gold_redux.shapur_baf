...
(if ((abs (_minAngle - _maxAngle)) < 330) then {
    ...
    _distMajor = (random 1) * (_XYSizeArray select 0) * ZEN_STD_MTH_RNP;
    _distMinor = (random 1) * ((_XYSizeArray select 1) * sqrt (1 - (_distMajor / (_XYSizeArray select 0))^2)) * ZEN_STD_MTH_RNP;
